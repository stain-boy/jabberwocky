...
#
# wget -qO ~/install.murmur https://git.io/vhYy0 && bash ~/install.murmur
#
############################
###### Basic Info End ######
############################
...
#
    ...
    echo
    apt-get download adduser debconf libvahi-client3 libvahi-common3 libavahi-compat-libdnssd1 libc6 lipcap2 libgcc1 libiceutil35 libqt4-dbus libqt4-network libqt4-sql libqt4-sql-sqlite libqt4-xml libqtcore4 libssl1.0.0 libstdc++6 libzeroc-ice35 dbus mumble-django mumble-server
    echo
    for f in $(ls)
    do
    	echo "Extracting $f"
